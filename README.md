...
SYS - Speak Your Story is a modern desktop notes application that lets you capture ideas effortlessly by speaking. It listens to your microphone, transcribes with a local Whisper model, and refines the transcript through your choice of **0MB RAM Built-in Offline Engine**, **Free Cloud AI API**, or **Local Ollama** — **all fully self-contained in a standalone executable with zero heavy LLM downloads required!**
...
| 🤖 **Multi-Engine Refinement** | Switch dynamically between ⚡ Built-in (0MB RAM), 🌐 Cloud AI (API), or 🦙 Local Ollama |
...
Microphone → WebRTC VAD → Local Whisper ASR → Raw Transcript
                 ...
                 │                                                                     │
                 ▼                                                                     ▼
    ⚡ Built-in Fast Engine                                               🌐 Cloud API / 🦙 Ollama
    (0MB RAM, Instant Rule-Based)                                          (Deep Context Cleanup)
                 │                                                                     │
                 └──────────────────────────────────┬──────────────────────────────────┘
                                                    │
                                                    ▼
...
3. **`src/refiner.py`**:
   ...
   - **`api`**: Free Cloud API (Gemini / Groq / OpenAI) for zero-memory cloud refinement.
...
*(No Ollama or LLM downloads needed! Runs out-of-the-box using the built-in fast engine.)*
...
│   ├── refiner.py          # Built-in 0MB RAM refiner + API + Ollama
